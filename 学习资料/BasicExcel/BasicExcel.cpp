...
ULONG Workbook::SharedStringTable::Read(const char* data)
{
	...
	// Fix by Ami Castonguay, 30.11.2016
//	return npos + 4*(npos/8224 + 1);
	...
	{
		// Need to insert 4 bytes for each continue indices which might not be a multiple of 8224 bytes for each chunks.
		return npos + 4*(continueIndices_.size() + 1);
	}
}
...
//	return recordSize_ = dataSize + 4*(dataSize/8224 + 1);
	...
	{
		// Need to insert 4 bytes for each continue indices which might not be a multiple of 8224 bytes for each chunks.
		return recordSize_ = dataSize + 4*(continueIndices_.size() + 1);
	}
}
