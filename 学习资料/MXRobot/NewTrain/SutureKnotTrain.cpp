...
//=============================================================================================
CSutureKnotTrain::CSutureKnotTrain(void)
{
    for (int i = 0; i < 2;i++)
    {
        m_Needle[i] = 0;
...
//======================================================================================================================
bool CSutureKnotTrain::Initialize(CXMLWrapperTraining * pTrainingConfig, CXMLWrapperToolConfig * pToolConfig)
{
    ...
    m_Needle[0] = CreateNeedle(MXOgre_SCENEMANAGER, m_SutureNodeNum, m_SutureRsLength, m_NeedleSkeletonFile);
    m_Needle[0]->addNeedleActionListener(this);
    SutureThread* attachrope = m_Needle[0]->GetSutureThread();
    if (attachrope)
    {
        m_Knot[0] = new SutureKnot();
		...
		if (0)
		{
			int min = 11;
			int max = 19;
			KnotInSutureRope deadknot = KnotInSutureRope(knotpbd(attachrope, min - 2, max), knotpbd(attachrope, min, max + 2));
			...
			m_Knot[0]->m_currentKnot = KnotInSutureRope(knotpbd(attachrope, min - 2, max), knotpbd(attachrope, min, max + 2));
...
//======================================================================================================================
void CSutureKnotTrain::InitBallPos()
{
    
}
//======================================================================================================================
bool CSutureKnotTrain::Update(float dt)
{
...
void CSutureKnotTrain::InternalSimulateStart(int currStep, int TotalStep, Real dt)
{
...
void CSutureKnotTrain::InternalSimulateEnd(int currStep, int TotalStep, Real dt)
{
...
void CSutureKnotTrain::OnCreateInAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{

}
//======================================================================================================================

void CSutureKnotTrain::OnCreateOutAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{

}
//======================================================================================================================

void CSutureKnotTrain::OnRemoveInAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{

}
//======================================================================================================================

void CSutureKnotTrain::OnRemoveOutAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{

}
//======================================================================================================================

void CSutureKnotTrain::OnSwitchAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{
    
}
//======================================================================================================================

void CSutureKnotTrain::OnRemoveRopeAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{

}
//======================================================================================================================

void CSutureKnotTrain::OnMovingRopeAnchor(const SutureThread * thread, const int index, const Real weights[])
{

}
//======================================================================================================================

GFPhysVector3 CSutureKnotTrain::CalcNormalFromCenter(SutureNeedle* needle, const TRCollidePair & pair, const GFPhysVector3 & Pos, const GFPhysVector3 & dir, GFPhysVector3 & foot)
{
    ...
    {
        foot = target;
        SutureRopeNode& refNode0 = needle->GetSutureThread()->GetThreadNodeRef(pair.m_Segment);
        SutureRopeNode& refNode1 = needle->GetSutureThread()->GetThreadNodeRef(pair.m_Segment + 1);

        GFPhysVector3 ponline = refNode0.m_CurrPosition * pair.m_w0 + refNode1.m_CurrPosition * pair.m_w1;
...
bool CSutureKnotTrain::checkCircleOnTool(SutureNeedle* needle, Real threshold)
{
    if (m_pToolsMgr->GetRightTool())
    {
        ...
        if (rightTool->m_centertoolpartconvex.m_rigidbody)
        {
            ...
            const GFPhysAlignedVectorObj<TRCollidePair> & TRPairs = needle->GetSutureThread()->GetCollidePairsWithRigid();
...
void CSutureKnotTrain::OnRigidClampByTool(GFPhysRigidBody * rigid)
{

}

void CSutureKnotTrain::OnThreadClampByTool(int ClampedSegGlobalIndex, CTool* toolobject)
{

}

void CSutureKnotTrain::OnThreadReleaseByTool(int ClampedSegGlobalIndex, CTool* toolobject)
{
    
}

void CSutureKnotTrain::OnRigidReleaseByTool(GFPhysRigidBody * rigid)
{

}
//======================================================================================================================

void CSutureKnotTrain::OnSimpleUIEvent(const SimpleUIEvent & event)
{
...
void CSutureKnotTrain::CheckAdjust_Tail()
{
    for (int i = 0; i < 2; i++)
    {
        if (!m_bAdjust_Tail[i])
        {
            ...
            if (m_Needle[i])
            {
                m_Needle[i]->GetFaceRopeAnchors(activeRopeAnchors);
...
void CSutureKnotTrain::CountThreadSeperate()
{
    for (int i = 0; i < 2; i++)
    {
        GFPhysVectorObj<GFPhysFaceRopeAnchor*> activeRopeAnchors;
...
void CSutureKnotTrain::ResetNeedle()
{
	if (m_pToolsMgr)
	{
    ...
    if (m_Needle[0] && m_Needle[1])
    {
        ...
        m_Needle[1] = CreateNeedle(MXOgre_SCENEMANAGER, m_SutureNodeNum, m_SutureRsLength, m_NeedleSkeletonFile);
        m_Needle[1]->addNeedleActionListener(this);

        SutureThread* attachrope = m_Needle[1]->GetSutureThread();
        if (attachrope)
        {
            m_Knot[1] = new SutureKnot();
...
bool CSutureKnotTrain1::Initialize(CXMLWrapperTraining * pTrainingConfig, CXMLWrapperToolConfig * pToolConfig)
{
    bool result = CSutureKnotTrain::Initialize(pTrainingConfig, pToolConfig);
...
void CSutureKnotTrain1::InitBallPos()
{
	for (int i = 0; i < 2;i++)
	{
...
void CSutureKnotTrain1::UpdateBaseBallPos()//更新定位球
{
...
void CSutureKnotTrain1::DrawMarkRenderTarget()
{
...
bool CSutureKnotTrain1::Update(float dt)
{
    bool result = CSutureKnotTrain::Update(dt);
    UpdateBaseBallPos();
    DrawMarkRenderTarget();

    GFPhysVectorObj<GFPhysFaceRopeAnchor*> activeRopeAnchors;
    m_Needle[0]->GetFaceRopeAnchors(activeRopeAnchors);
    if (activeRopeAnchors.size() == 0)
    {
	...
	SutureThread* thread = m_Needle[0]->GetSutureThread();
	if (thread)
	{
		GFPhysVectorObj<KnotInSutureRope> Knots;
...
void CSutureKnotTrain1::OnThreadClampByTool(int ClampedSegIndex, CTool* toolobject)
{
    CSutureKnotTrain::OnThreadClampByTool(ClampedSegIndex, toolobject);
    //绕圈成功+抓住近线尾
    GFPhysVectorObj<GFPhysFaceRopeAnchor*> activeRopeAnchors;
    m_Needle[0]->GetFaceRopeAnchors(activeRopeAnchors);
    if (activeRopeAnchors.size() > 0)
...
void CSutureKnotTrain1::OnRigidClampByTool(GFPhysRigidBody * rigid)
{
...
void CSutureKnotTrain1::OnRigidReleaseByTool(GFPhysRigidBody * rigid)
{
    if (rigid == m_Needle[0]->GetPhysicBody())
    {
        m_bGraspNeedle = false;
...
void CSutureKnotTrain1::OnThreadReleaseByTool(int ClampedSegGlobalIndex, CTool* toolobject)
{
    CSutureKnotTrain::OnThreadReleaseByTool(ClampedSegGlobalIndex, toolobject);
    GFPhysVectorObj<GFPhysFaceRopeAnchor*> activeRopeAnchors;
    m_Needle[0]->GetFaceRopeAnchors(activeRopeAnchors);
    if (activeRopeAnchors.size() > 0)
    {
...
void CSutureKnotTrain1::ResetNeedle()
{
    __super::ResetNeedle();
...
void CSutureKnotTrain1::OnSaveTrainingReport()
{
...
{	
    ...
    GFPhysVectorObj<GFPhysFaceRopeAnchor*> ropefaceanchors;
    m_Needle[0]->GetFaceRopeAnchors(ropefaceanchors);
	...
	GFPhysFaceRopeAnchor* anchor0 = new GFPhysFaceRopeAnchor(face0, weights0, m_Needle[0]->GetSutureThread(), n0, threadweights0);

	GFPhysFaceRopeAnchor* anchor1 = new GFPhysFaceRopeAnchor(face1, weights1, m_Needle[0]->GetSutureThread(), n1, threadweights1);

    ropefaceanchors.push_back(anchor0);
    ropefaceanchors.push_back(anchor1);

    m_Needle[0]->SetFaceRopeAnchors(ropefaceanchors);
}

//CSutureKnotTrain1::~CSutureKnotTrain1(void)
//{
//    CSutureKnotTrain::~CSutureKnotTrain();
//}
//======================================================================================================================

bool CSutureKnotTrain2::Initialize(CXMLWrapperTraining * pTrainingConfig, CXMLWrapperToolConfig * pToolConfig)
{
    bool result = CSutureKnotTrain1::Initialize(pTrainingConfig, pToolConfig);   

	SutureThread* rope = m_Needle[0]->GetSutureThread();
...
bool CSutureKnotTrain2::Update(float dt)
{
    bool result = CSutureKnotTrain1::Update(dt);

    KnotInSutureRope & Knot = m_Needle[0]->GetSutureThread()->m_KnotsInThread->GetCurrKnot();
...
//======================================================================================================================
bool CSutureKnotTrain3::Initialize(CXMLWrapperTraining * pTrainingConfig, CXMLWrapperToolConfig * pToolConfig)
{
    bool result = CSutureKnotTrain::Initialize(pTrainingConfig, pToolConfig);
	...
	if (false)
	{
		m_sutureV1 = new SutureThread(MXOgre_SCENEMANAGER, this);
		...
		m_sutureV1->CreateFreeThread(StartFixPoint, EndFixPoint, segmentCount, masspernode);

		SutureRopeNode& node0 = m_sutureV1->GetThreadNodeRef(0);
...
//======================================================================================================================
void CSutureKnotTrain3::InitBallPos()
{
    for (int i = 0; i < 8; i++)
    {
...
//======================================================================================================================
void CSutureKnotTrain3::ResetNeedle()
{
    m_ballinfo.clear();
    __super::ResetNeedle(); 
    m_NeedleFallStateLast = false;
...
//======================================================================================================================
void CSutureKnotTrain3::CreateAnotherNeedle()
{
    m_Needle[1] = CreateNeedle(MXOgre_SCENEMANAGER, m_SutureNodeNum, m_SutureRsLength, m_NeedleSkeletonFile);
    m_Needle[1]->addNeedleActionListener(this);
    SutureThread* attachrope = m_Needle[1]->GetSutureThread();
...
//======================================================================================================================
void CSutureKnotTrain3::DrawMarkRenderTarget()
{
...
//======================================================================================================================
bool CSutureKnotTrain3::Update(float dt)
{
    bool result = CSutureKnotTrain::Update(dt);
	...
	m_sutureV1->PrepareSolveConstraint(1.0f, 0.01f);
	m_sutureV1->SolveConstraint(1.0f, 0.01f);
    ...
    if (m_bTrainingIlluminated)
    {
        ...
        if (m_Needle[0] && m_Needle[1])
        {
            m_Needle[1]->GetFaceRopeAnchors(activeRopeAnchors);

            GFPhysVectorObj<FaceAnchorInfo>& faceanchors = m_Needle[1]->getFaceNeedleAnchors();
...
//======================================================================================================================
void CSutureKnotTrain3::UpdateBaseBallPos()
{
	for (int i = 0, ni = m_ballinfo.size(); i < ni; i++)
    {
...
//======================================================================================================================
void CSutureKnotTrain3::OnCreateInAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{
		...
		for (size_t p = 0, np = rightTool->m_ToolPlugins.size(); p < np; p++)
        {
            ...
            if (holdPlugin && holdPlugin->GetRigidHoldState())
            {
                ...
                if (m_Needle[1] == NULL)
                {
					...
					GFPhysFaceNeedleAnchor* lastAnchor = faceanchors[k - 1].pAnchor;
...
//======================================================================================================================
void CSutureKnotTrain3::OnCreateOutAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{
    GFPhysVector3 anchorpos = face->m_Nodes[0]->m_CurrPosition * weights[0]
        + face->m_Nodes[1]->m_CurrPosition * weights[1]
        + face->m_Nodes[2]->m_CurrPosition * weights[2];
...
//======================================================================================================================
void CSutureKnotTrain3::OnRemoveInAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{
    GFPhysVector3 anchorpos = face->m_Nodes[0]->m_CurrPosition * weights[0]
        + face->m_Nodes[1]->m_CurrPosition * weights[1]
        + face->m_Nodes[2]->m_CurrPosition * weights[2];
...
//======================================================================================================================
void CSutureKnotTrain3::OnRemoveOutAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{
    GFPhysVector3 anchorpos = face->m_Nodes[0]->m_CurrPosition * weights[0]
        + face->m_Nodes[1]->m_CurrPosition * weights[1]
        + face->m_Nodes[2]->m_CurrPosition * weights[2];
...
//======================================================================================================================
void CSutureKnotTrain3::OnRemoveRopeAnchor(const GFPhysSoftBodyFace* face, const Real weights[])
{
    GFPhysVector3 anchorpos = face->m_Nodes[0]->m_CurrPosition * weights[0]
        + face->m_Nodes[1]->m_CurrPosition * weights[1]
        + face->m_Nodes[2]->m_CurrPosition * weights[2];
...
//======================================================================================================================
void CSutureKnotTrain3::UpdateStaticMark(const GFPhysVector3& anchorpos)
{
...
//======================================================================================================================
void CSutureKnotTrain3::CreatedynamicMark(const GFPhysSoftBodyFace* face, const Real weights[])
{
...
//======================================================================================================================
void CSutureKnotTrain3::DeletedynamicMark(const GFPhysSoftBodyFace* face)
{
...
//======================================================================================================================
void CSutureKnotTrain3::OnThreadClampByTool(int ClampedSegIndex, CTool* toolobject)
{
    CSutureKnotTrain::OnThreadClampByTool(ClampedSegIndex, toolobject);
    //绕圈成功+抓住近线尾
    GFPhysVectorObj<GFPhysFaceRopeAnchor*> activeRopeAnchors;
...
void CSutureKnotTrain3::OnRigidClampByTool(GFPhysRigidBody * rigid)
{
    for (int i = 0; i < 2; i++)
    {
...
void CSutureKnotTrain3::OnRigidReleaseByTool(GFPhysRigidBody * rigid)
{
    for (int i = 0; i < 2; i++)
    {
        if (m_Needle[i] && rigid == m_Needle[i]->GetPhysicBody())
...
void CSutureKnotTrain3::OnThreadReleaseByTool(int ClampedSegGlobalIndex, CTool* toolobject)
{
    CSutureKnotTrain::OnThreadReleaseByTool(ClampedSegGlobalIndex, toolobject);
...
void CSutureKnotTrain3::OnCollisionKeep(GFPhysCollideObject * objA, GFPhysCollideObject * objB, const GFPhysManifoldPoint * contactPoints, int NumContactPoints)
{
...
void CSutureKnotTrain3::DetectNeedleFall()
{
...
void CSutureKnotTrain3::GatherPosInfo()
{
    for (int i = 0; i < 2;i++)
    {
...
void CSutureKnotTrain3::OnSaveTrainingReport()
{
