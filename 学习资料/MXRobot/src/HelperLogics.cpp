...
public:
	...
	virtual void notifyMaterialSetup(Ogre::uint32 pass_id, Ogre::MaterialPtr &mat);
	virtual void notifyMaterialRender(Ogre::uint32 pass_id, Ogre::MaterialPtr &mat);
	Ogre::MaterialPtr m_FinalToneMatPtr;
	Ogre::MaterialPtr m_BrightpassPtr;
	//for ssss
...
//---------------------------------------------------------------------------
Ogre::MaterialPtr HDRLogic::GetBrightpassPtr()
{
	if(m_listener)
	{
		return m_listener->m_BrightpassPtr;
	}
}

Ogre::MaterialPtr HDRLogic::GetFinalToneMatPtr()
{
	if(m_listener)
	{
		return m_listener->m_FinalToneMatPtr;
	}
}
Ogre::MaterialPtr HDRLogic::GetSSSSBlurXMatPtr()
{
	if(m_listener)
	{
		return m_listener->m_SSSSBlurXMatPtr;
	}
}
Ogre::MaterialPtr HDRLogic::GetSSSSBlurYMatPtr()
{
	if(m_listener)
	{
		return m_listener->m_SSSSBlurYMatPtr;
	}
}
...
void HDRListener::notifyMaterialSetup(Ogre::uint32 pass_id, Ogre::MaterialPtr &mat)
{
	...
	case 800: 
		{
			m_BrightpassPtr = mat;// rt_brightpass
			break;
		}
	case 701: // rt_bloom1
		{
	...
	case 666:
		{
			m_FinalToneMatPtr = mat;
			break;
		}
	case 333:
		{
			m_SSSSBlurXMatPtr = mat;
			break;
		}
	case 555:
		{
			m_SSSSBlurYMatPtr = mat;
			break;
		}
	}
}
