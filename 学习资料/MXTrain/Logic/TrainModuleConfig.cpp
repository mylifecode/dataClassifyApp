				...
				while (!subModuleNode.isNull())
				{
					...
					while (!trainItemNode.isNull())
					{
						...
						if (m_TrainItemMap.find(strid) != m_TrainItemMap.end())
						{
						    ...
						    m_TrainItemMap.insert(std::make_pair(strid, TrainModuleConfig::TrainItemAttribute(strid, globalName, showName, casefile, videofile)));
...
bool TrainModuleConfig::GetMenuAttributeById(const QString & id, QString & globalName, QString & showName, QString & casefile, QString & videoFile)
{
