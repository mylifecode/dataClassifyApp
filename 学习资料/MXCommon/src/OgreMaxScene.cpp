...
int OgreMaxScene::s_nLoadCount = 0;
...
void OgreMaxScene::Update(Real elapsedTime)
{
        ...
        ++stateIterator)
    {
		...
		{
			AnimationStatesSpeed::iterator speedIter=animationStatesSpeed.find(animationState->getAnimationName());
			Ogre::Real speed=1.0;
			if (speedIter!=animationStatesSpeed.end())
			{
				speed=speedIter->second;
			}
			animationState->addTime(elapsedTime*speed);
		}         
    }
}
...
Light* OgreMaxScene::GetLight(const String& name, bool nameIncludesPrefix)
{
	Light* light = 0;
	...
	if (nameIncludesPrefix)
	{
		...
		if (this->sceneManager->hasLight(name))
			light = this->sceneManager->getLight(name);
	}
	else
	{
		...
		String prefixedName = this->objectNamePrefix;
		prefixedName += name;
		if (this->sceneManager->hasLight(prefixedName))
			light = this->sceneManager->getLight(prefixedName);
	}
...
void OgreMaxScene::SetAnimationState(const Ogre::String& animationName,Ogre::AnimationState* animateState)
{
	animationStates[animationName]=	animateState;
}

void OgreMaxScene::SetAnimationStateSpeed(const Ogre::String& animationName,Ogre::Real animateStateSpeed)
{
	animationStatesSpeed[animationName]=animateStateSpeed;
}

Ogre::Real OgreMaxScene::GetAnimationStateSpeed(const Ogre::String& animationName)
{
	AnimationStatesSpeed::iterator stateIterator = animationStatesSpeed.find(animationName);
	if (stateIterator != animationStatesSpeed.end())
		return stateIterator->second;
	else
		return 1.0;
}
...
String OgreMaxScene::GetNewObjectName(const TiXmlElement* objectElement, SceneNode* node)
{
	...
	sprintf_s(szLoadCount, "%d", s_nLoadCount);

	//zx
	name = name + "$" + szLoadCount; 
...
//不起实际作用，这个配置在所有scene都未使用，删除于20150807 wang xingmin，方便日后要得时候恢复
//LoadSahdows()
...
//         this->callback->CreatedMesh(this, mesh.getPointer());
    ...
    //Create entity
	static int xingmin_count = 0;
	xingmin_count++;
	//if ( parameters.meshFile == "appendectomy_large_intestine.mesh" )//为调试只加载mesh
	//	return;
	...
	//create EdgeList for Shadows Effect by yl
	if (!entity->hasEdgeList())
	{
...
// 
	Ogre::AnimationStateSet* animationSet = entity->getAllAnimationStates(); 
	if(animationSet)    
	{
		Ogre::AnimationStateIterator iter = animationSet->getAnimationStateIterator();
		while(iter.hasMoreElements()) 
		{
			Ogre::AnimationState* animation = iter.getNext();
			AnimationStates::iterator stateIterator = animationStates.find(animation->getAnimationName());
			if (stateIterator == animationStates.end())
			{
				animationStates[animation->getAnimationName()]=animation;
				animationStatesSpeed[animation->getAnimationName()]=1.0;
			}
		}
	}
}
...
//         this->callback->CreatedNodeAnimationTrack(this, node, animationTrack, params.enable, params.looping);
    ...
    if ((this->loadOptions & NO_ANIMATION_STATES) == 0)
    {
        ...
        if (GetAnimationState(params.name) == 0)
        {
			...
			this->animationStatesSpeed[params.name]=1.0;
