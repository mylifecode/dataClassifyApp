...
CNvHWEncoder::CNvHWEncoder()
{
	...
	m_videoWriter = new MxVideoWriter;

	memset(&m_sequenceParam, 0, sizeof(m_sequenceParam));
	memset(m_spsppsInfo, 0, sizeof(m_spsppsInfo));
	m_spsppsLength = 0;
}
...
NVENCSTATUS CNvHWEncoder::CreateEncoder(const EncodeConfig *pEncCfg)
{
	...
	//the following is for use net work see http://stackoverflow.com/questions/33185966/how-to-stream-h-264-video-over-udp-using-the-nvidia-nvenc-hardware-encoder
#if(0)
	...
	m_spsppsLength = 0;
	memset(&m_sequenceParam, 0, sizeof(m_sequenceParam));
	m_sequenceParam.version = NV_ENC_SEQUENCE_PARAM_PAYLOAD_VER;
	m_sequenceParam.inBufferSize = SPS_PPS_BUFFER_SIZE;
	m_sequenceParam.spsppsBuffer = m_spsppsInfo;
	m_sequenceParam.outSPSPPSPayloadSize = &m_spsppsLength;
	nvStatus = NvEncGetSequenceParams(&m_sequenceParam);

	//set attribute for video writer
	m_videoWriter->SetBitRate(pEncCfg->bitrate);
	m_videoWriter->SetSize(pEncCfg->width, pEncCfg->height);
	m_videoWriter->SetFrameRate(pEncCfg->fps);
	m_videoWriter->SetPixFormat(AV_PIX_FMT_NV12);
	m_videoWriter->SetGroupSize(m_stCreateEncodeParams.encodeConfig->gopLength);
	m_videoWriter->SetNumberOfMaxBFrame(0);
	if(m_spsppsLength > 0)
		m_videoWriter->SetSPSPPS((char*)m_spsppsInfo, m_spsppsLength);
	m_videoWriter->Open(m_fileName);
...
NVENCSTATUS CNvHWEncoder::ProcessOutput(const EncodeBuffer *pEncodeBuffer)
{
	...
	if (nvStatus == NV_ENC_SUCCESS)
	{
		...
		//write data
		m_videoWriter->WriteData(lockBitstreamData.outputTimeStamp, lockBitstreamData.outputTimeStamp,
