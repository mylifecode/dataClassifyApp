...
namespace MX
{
	...
	void DSS_LIB login(char *username, char *pwd, LoginCallBack callback)
	{
		...
		{
			QString _name = reply["UserName"].toString();
			QByteArray byteArray = _name.toLocal8Bit();
			char* pszName = byteArray.data();

			bool ok = reply["ErrorCode"].toBool();
			int mode = reply["Mode"].toInt();
			char* username = reply["UserName"].toString().toLocal8Bit().data();

			auto na = reply["Name"].toString().toLocal8Bit();
			char* name = na.data();

			auto ti = reply["Trainitem"].toString().toLocal8Bit();
			char* trainitem = ti.data();
			callback(ok, mode, pszName, name, trainitem);
